...
JITServerAOTDeserializer::findInMap(const PersistentUnorderedMap<uintptr_t, V> &map, uintptr_t id, TR::Monitor *monitor, TR::Compilation *comp, bool &wasReset)
   ...
   // TODO: assert here that the record ID must have been found?
   return V();
   }
...
// Free all the values (which must have been allocated with TR::Compiler->persistentGlobalMemory()->allocatePersistentMemory)
// to the given map. Note that this leaves invalid pointers in the map, so the map should be cleared after this operation.
template<typename K, typename V, typename H> static void
freeChainMapValues(PersistentUnorderedMap<K, V *, H> &map)
   {
   for (auto &kv : map)
...
JITServerNoSCCAOTDeserializer::clearCachedData()
   ...
   freeChainMapValues(_classChainMap);
   _classChainMap.clear();

   freeChainMapValues(_wellKnownClassesMap);
   _wellKnownClassesMap.clear();

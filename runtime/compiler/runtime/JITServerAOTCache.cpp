...
const AOTCacheThunkRecord *
JITServerAOTCache::storeThunk(const uint8_t *signature, uint32_t signatureSize, const uint8_t *thunkStart, uint32_t thunkSize)
   {
   if (!JITServerAOTCacheMap::cacheHasSpace())
      return NULL;

   auto record = AOTCacheThunkRecord::create(_nextThunkId, signature, signatureSize, thunkStart, thunkSize);
   addToMap(_thunkMap, _thunkHead, _thunkTail, getRecordKey(record), record);
   ++_nextThunkId;

...
static bool
writeAOTRecordList(FILE *f, const AOTCacheRecord *head, size_t numRecordsToWrite)
   {
...
JITServerAOTCache::writeCache(FILE *f) const
   ...
   if (!writeAOTRecordList(f, _classHead, header._numClassRecords))
      return false;
   if (!writeAOTRecordList(f, _methodHead, header._numMethodRecords))
      return false;
   if (!writeAOTRecordList(f, _classChainHead, header._numClassChainRecords))
      return false;
   if (!writeAOTRecordList(f, _wellKnownClassesHead, header._numWellKnownClassesRecords))
      return false;
   if (!writeAOTRecordList(f, _aotHeaderHead, header._numAOTHeaderRecords))
      return false;

...
uint8_t *TR::PPCForceRecompilationSnippet::emitSnippetBody()
   ...
   intptr_t startPCRegValue = cg()->canEmitDataForExternallyRelocatableInstructions() ? startPC : 0;
   TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "We're emitting a snippet under %s with startPCRegValue: %lu and startPC value: %lu",
                                  cg()->comp()->compileRelocatableCode() ? "AOT" : "non-AOT",
                                  startPCRegValue,
                                  startPC);

...
void *
TR_J9ServerVM::getClientThunk(std::string signature, TR::Compilation *comp)
   {
      {
...
TR_J9ServerVM::getJ2IThunk(char *signatureChars, uint32_t signatureLength, TR::Compilation *comp)
   ...
   if (comp->isAOTCacheStore())
      ...
      if (record)
         ...
         if (!getClientThunk(signature, comp))
            serializeClientThunk(signature, record->data().thunkStart(), record->data().thunkSize(), comp);
         }
      }
   else
      {
      thunkPtr = getClientThunk(signature, comp);
      }
...
void
TR_J9ServerVM::serializeClientThunk(std::string signature, const uint8_t *thunkStart, const uint32_t thunkSize, TR::Compilation *comp)
   {
...
TR_J9ServerVM::setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr, TR::Compilation *comp)
   ...
   if (comp->isAOTCacheStore())
      {
      auto record = comp->getClientData()->getAOTCache()->storeThunk((uint8_t *)signatureChars, signatureLength, thunkStart, thunkSize);
      comp->addThunkRecord(record);

      if (!getClientThunk(signature, comp))
         serializeClientThunk(signature, thunkStart, thunkSize, comp);
      }
   else
      {
      serializeClientThunk(signature, thunkStart, thunkSize, comp);

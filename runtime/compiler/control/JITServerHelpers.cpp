...
ROMSegmentMap::registerDebugInfoSegment(const J9ROMClass *romClass, const uint8_t *debugInfoSegmentStart, size_t debugInfoSegmentSize)
   ...
   // ROM class uniformly by _accumulatedShift.
...
struct ROMClassPackContext
   {
   ROMClassPackContext(TR_Memory *trMemory, const J9ROMClass *romClass, size_t origSize, bool useSimpleMethodNormalization) :
      _segmentMap(trMemory, romClass),
      _origRomClassStart((const uint8_t *)romClass), _useSimpleMethodNormalization(useSimpleMethodNormalization), _origSize(origSize),
   ...
   bool _useSimpleMethodNormalization;
...
ROMClassPackContext::newAddressFromOld(const uint8_t *oldAddr) const
   ...
   // If the ROM class is shared, the offset can be transferred directly
   if (_useSimpleMethodNormalization)
...
JITServerHelpers::packROMClass(J9ROMClass *romClass, TR_Memory *trMemory, TR_J9VMBase *fej9, size_t &packedSize, size_t expectedSize)
   ...
   // Shared ROM classes have their return bytecodes fixed when they are created and have their debug
   // info stored totally out-of-line, so we can perform simpler normalization for them.
   // Array ROM classes don't have ROM methods at all. ROM classes not in one of these two categories
   // need to have the return bytecodes in their ROM methods fixed and their (potentially inline)
   // debug info stripped.
   bool isSharedROMClass = fej9->sharedCache() && fej9->sharedCache()->isROMClassInSharedCache(romClass);
   bool needsDeepMethodNormalization = !(isSharedROMClass || isArrayROMClass(romClass));
   ...
   ROMClassPackContext ctx(trMemory, romClass, origRomSize, !needsDeepMethodNormalization);

   // Populate the segment map
   J9ROMMethod *romMethod = J9ROMCLASS_ROMMETHODS(romClass);
   if (needsDeepMethodNormalization)
      {
   ...
   // Non-shared, non-array ROM classes had their debug info stripped and zeroed
   // out when the pre-string section of the ROM class was copied above
   if (!needsDeepMethodNormalization)
      {
      ...
      for (size_t i = 0; i < ctx._packedRomClass->romMethodCount; ++i)
         ...
         if (J9ROMMETHOD_HAS_DEBUG_INFO(romMethod))
            {
            auto debugInfo = methodDebugInfoFromROMMethod(romMethod);
            TR_ASSERT_FATAL(!(debugInfo->srpToVarInfo & 1), "Shared ROM class %*.s has inline debug info", name->length, name->data);
   ...
   if (isArrayROMClass(romClass))
      ...
      {
      // 2nd pass: copy all strings to their locations in the packed ROMClass, and, if the ROM class isn't shared,
      // adjust all SRPs that were invalidated by discarding the method debug info.
      ctx._callback = packCallback;
      ctx._srpCallback = isSharedROMClass ? NULL : adjustSRPCallback;
      ctx._wsrpCallback = isSharedROMClass ? NULL : adjustWSRPCallback;
      allSlotsInROMClassDo(romClass, slotCallback, NULL, NULL, &ctx);
      }

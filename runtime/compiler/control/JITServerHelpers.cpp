...
ROMClassPackContext::addSegVec(const uint8_t *addr, const char *name)
   ...
   auto romname = J9ROMCLASS_CLASSNAME((J9ROMClass *)_origRomClassStart);
...
// be 64-bit aligned, so padding (I believe up to 12 bytes) may have been added between the last pre-string
// section and the UTF8 string section. When we strip out the debug info from a ROM class, we might reintroduce
// a misalignment in this pre-string section. Fixing this misalignment might require us to add more padding, or
// to drop some of the padding that was already added during ROM class writing. We can only decide to do one or
// the other if we know the unpadded size of the pre-string section. Once we have this information, we can calculate
// the aligned and padded size of the pre-string section (currently called the packedNonStringSize) properly in packROMClass.
static void
